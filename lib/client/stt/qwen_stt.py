"""1차 전사 (Qwen3-ASR) — worker-prep_stt 의 stt_svc 호출. (transport)

  POST STT_URL  SttRequest → segments

같은 서버의 2차 전사(whisper)는 whisper_stt.py — 둘이 base URL 을 공유한다.
...
def transcribe(http: httpx.Client, vid: int, file_path: str) -> list[dict]:
    """1차 전사 — POST STT_URL 동기 호출. 5~10분 블로킹 후 segments 반환."""
