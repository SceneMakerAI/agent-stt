"""교정 LLM 콜 — 페이지 1개 교정. 프롬프트도 여기 산다.

vLLM 에 가는 건 이 파일이 전부다. 페이지 분할·취합은 util_correct, 흐름은 pipe_correct.

build() 가 [공통 SYSTEM] + [영상 정보] + [참고 용어](용어집) + [참고 명단](roster) 을 조립해
chat messages 를 만든다.

⚠ **카테고리를 해석하지 않는다.** 용어집은 이미 고른 텍스트로 받는다(glossary 인자).
   어느 카테고리에 어느 용어집을 줄지는 프로파일(카테고리 표)의 일이다.

원칙:
  - LLM 에는 메타(타임코드/화자)를 주지 않는다. idx + 본문(text) 만.
  - 줄 수/순서/idx 를 그대로 유지하도록 강제 → merge 가 idx 로 메타 복원.
  - 출력은 JSON 만. (pipe_correct 가 파싱)
"""
import json
...
# 용어집(glossary_correct 의 목록)에 씌우는 머리말. 목록 자체는 자산이라 저기 두고,
# '어떻게 쓰라'는 지시는 프롬프트라 여기 둔다.
GLOSSARY_HEADER = """[참고 용어]
...
def _video_info(req) -> str:
    ...
    카테고리를 '분기'에 쓰는 게 아니라 프롬프트에 텍스트로 싣는 것뿐이다.
    """
    if req is None:
        return ""
    info = []
...
def build(page: list[Dialogue], roster: str = "", req=None,
          whisper_map: dict[int, str] | None = None, glossary: str = "") -> list[dict]:
    """페이지(대사 리스트) → chat messages. text 와 idx 만 노출.

    req(영상 정보)·glossary(용어집 텍스트)·roster(명단)가 있으면 교정 참고자료로 system 에
    덧붙인다. whisper_map 이 있으면 해당 idx 줄에 2차 전사를 [B] 로 병기한다
    (대조 규칙은 SYSTEM 에 상주). 없는 섹션은 자동 생략.
    """
    ...
    if info:
        parts.append(info)
    if glossary.strip():
        parts.append(f"{GLOSSARY_HEADER}\n\n{glossary.strip()}")
    if roster.strip():
...
async def correct_page(vllm: VLLMClient, page: list[Dialogue], roster: str = "", req=None,
                       whisper_map: dict[int, str] | None = None,
                       glossary: str = "") -> dict[int, str]:
    ...
    try:
        text, ms = await vllm.chat(
            messages=build(page, roster, req, whisper_map, glossary),
