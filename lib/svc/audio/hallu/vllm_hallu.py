"""할루시 판정 LLM 콜 — 언어이탈 후보 + 문맥 → 3분류 JSON. 프롬프트도 여기 산다.

vLLM 에 가는 건 이 파일이 전부다. 규칙 판정은 util_hallu.py, 흐름은 pipe_hallu.py.

pipe 가 '주언어와 다른 줄'을 후보로 뽑아, 각 후보를 **앞뒤 문맥과 함께** 여기로 보낸다.
...
def build(main: str, candidates: list[Dialogue], id2seg: dict[int, Dialogue],
          ctx: int = 2) -> list[dict]:
    ...
    for c in candidates:
        ...
        for j in range(i - ctx, i + ctx + 1):
            ...
            if not s:
                continue
            mark = " ← 후보" if j == i else ""
            lines.append(f'  [{j}] ({s.lang}) {s.text}{mark}')
        blocks.append("\n".join(lines))
...
async def judge(vllm: VLLMClient, main: str, candidates: list[Dialogue],
                id2seg: dict[int, Dialogue]) -> dict[int, str]:
