...
트랜잭션(connect)은 save_svc 가 연다. 여기는 '무엇을 넣을지'만 안다:
  t_frame_baseball           ← 프레임별 분류 (야구/광고·투구·보드종류 + 필수항목 개수)
...
"""
from lib.client.rdb import (t_frame_baseball, t_frame_board, t_frame_board_detail,
                            t_video_board)
...
def save(cur, v_id: int, out) -> int:
    ...
    n_adv = t_frame_baseball.delete_insert(cur, v_id, out.video)
