...
갈래(pipe_audio 등)가 각 단계 끝에서 dump(v_id, step, data) 한 줄로 호출.
...
def _enc(obj):
    """json 이 모르는 것 — 공정이 주고받는 dataclass(Dialogues/Summary 등)를 dict 로."""
    if is_dataclass(obj):
        return asdict(obj)
    raise TypeError(f"덤프 불가 타입: {type(obj).__name__}")
...
def dump(v_id: int, step: str, data) -> None:
    ...
    if isinstance(data, str):
        ...
        path = d / f"{step}.json"
        path.write_text(json.dumps(data, ensure_ascii=False, indent=2, default=_enc),
                        encoding="utf-8")
